...
I had an unused Pi 4 in my electronics collection that paired perfectly with this extra wide touchscreen display module that even includes mounting hardware to install a Pi directly onboard from Amazon. I've never worked with OpenCV before however I was able to leverage this powerful library to calibrate my camera and employ the derived coefficients to flatten every image read from the backup cam before showing it on the screen. I assembled a simple frame out of scrap aluminum L brackets and attached it using the screw mounting my [wireless phone charger](https://www.amazon.com/MCQVP-XTET-MagicMount-Magnetic-Qi-Certified-FreeFlow/dp/B07Z7CYRD2?th=1) to this [Clearmounts](https://www.audiphoneholder.com/product/78/clearmounts-bracket-low-profile-magnetic-holder-part-8p-low) product secured between the center dashboard vents. My efforts to avoid modifying the interior were for naught because I had to rip apart most of the floor and trunk trim in order to run the video cable once I validated my camera installation then drilled a big hole in the center console for the system power button. I soon added an OBDII reader for communication with the car, allowing me to project real-time engine information such as intake boost pressure on the screen. I am in the process of migrating that functionality to a secondary screen utilizing an RP2040 microcontroller to replicate a boost gauge but that is still very much a [work in progress](https://github.com/eligold/BOOSTGAUGE2040) A second USB camera module mounted to a DIN rail that also supports the 12V to 5V converter acts as a dash cam which should come in handy in the event of a traffic incident. You can see the actual boot sequence and an example of the automatic bluetooth connection [here](https://youtube.com/shorts/Pqzc3IFau1s)
...
1. ~~Icon for indicating when the USB storage drive is filling up~~ ✓
1. Implement backup battery with smart charging and fail-safe BMS
1. Custom PCB(s) to consolidate power, ~~digitization~~, OBD connection, and wiring
...
[![y u no load 4rings](./assets/pi_files/IMG_5046.PNG)](https://wall.alphacoders.com/big.php?i=474466)
